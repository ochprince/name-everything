...
2. **失败页误点陷阱** — 答错或超时后页面会快速切到失败页，而失败页的「下一句」按钮与四个选项处于同一位置；用户着急点选项时若页面已因失败切换，点击会落到「下一句」上，导致来不及查看正确答案。
3. **想法：难度模式** —
   - 困难模式：每个选项只有一次错误机会。
   - 地狱模式：每个选项没有错误机会。
4. **挑战模式（限时闯关）调整** — 保持 30 道题、6 组（不再缩减为 20/4）。难度曲线改为分层抽样 + 难度切组（2026-08-25 已实施）：句子按 章节→关卡 排序建立难度序；每关保底 1 句，剩余名额按难度分层各抽 1 句；抽出的句子按难度序切成 5 句一组（组 1 = 最早章节 → 组 6 = 最晚章节），组内随机、组间严格递增——后面分组不再混入前面简单句章节的例句。
5. **结算页与中间页动画** — 结算页和中间（失败）页加入动画效果：成功时放烟花等庆祝动画，失败时做与之相反、略带小失落的动画，增加页面趣味性（现在页面比较呆板）。
6. **「我的」设置项分组** — 现在「我的」里有很多配置项，但部分属于记忆单词模块、部分属于语法学习与挑战模式；应按模块对配置项做章节/分块分组，让页面更优雅清晰（后续语法游戏模式、难度选择也可能放进配置项，数量增多后不分组会很混乱）。备选方案：重新设计「我的」页面，把设置做成独立单页，由「我的」提供一个设置入口点击进入——这也是大多数 APP 的实现方式。
