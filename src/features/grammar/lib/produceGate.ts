...
export function buildProduceJudgePrompt(input: {
  ...
  const instructions = [
    ...
    '合格条件：',
    '1) 正确运用了本关语法知识点；',
    '2) 语法正确、语义自然（母语者能接受、听懂即可），不追求用词精妙或书面化；',
    '3) 不得逐字照抄课包例句或本局已合格句。内容相近没关系：只要换了人物/物品/场景就是合格的新句——练习的价值就在于反复造同类句。',
    '',
    '判定原则（重要）：',
    '- 找不到明确、具体的语法错误就判合格（pass=true）。宁可放过自然表达，也不要误伤正确的句子。',
    '- 不因用词、搭配或表达风格挑剔而判不合格。例如“给某人带某物”用 take 的双宾语结构（She takes me a bottle. = She takes a bottle to me.）语法正确；give/send/bring/take/hand/show/teach 等动词都可以带双宾语，不要臆造规则。',
    '- 不合格（pass=false）时，reason 必须能明确指出具体的语法问题（结构没用上、时态、语序、缺成分等）；找不出具体问题时不要判 fail。',
    '',
    `知识点标题：${input.titleZh}`,
    `知识点说明：${input.bodyZh}`,
    '课包例句（仅供对照语法结构，不是标准答案，也不要求你写出不同的内容）：',
    samples,
    ...(avoid
      ? [
          '',
          '本局学习者已写过的合格句（新的句子不得逐字重复；换人物/物品/场景即可）：',
          avoid,
        ]
      : []),
    '',
