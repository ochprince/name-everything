...
5. **Challenge mode is still recognition, not substitution** — the challenge mode (and level drills) currently test mastery of existing example sentences: recognize the structure, fill blanks in a known sentence. That is recognition — grammar learning + example learning. Substitution is the real goal: actually using the grammar to build new sentences (swap subject/object/verb to generate novel ones), not memorizing examples. Challenge mode should evolve toward substitution — e.g. generative prompts (grammar skeleton + Chinese meaning, learner produces the English sentence).
