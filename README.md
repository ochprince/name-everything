...
- **词汇记忆** — 看图，倒计时里先想那句英文，再 Aha! / Forgot / Got it。词汇按学习价值排序（动词>名词>代词&冠词>介词>形容词>副词，同词性内按词频降序；兼类词按 Brown 语料中实际最高频的**主导词性**归组，句法与主干策略/口语速成理念），每批约 10 词、批内出示随机防死记，每次练习 10 个词汇，Forgot 进复习；词汇记忆页不设次级入口（避免与报错/计数拥挤）；接龙玩法放在挑战模式第四门。揭示后可把例句「加入我的挑战」，也可对整词报错。
- **语法学习** — 每关先读一句生动的标杆句，哪里不会点哪里；然后玩游戏过关（限时选择或输入英文句子）。已开通 AI 时，清完练习句后多一道举一反三开放造句（已过关重打也会出现），AI 判定合格才进结算。
- **挑战模式** — 语法过关或收藏了词汇例句后开门。可选 **语法挑战**（已学句子综合局）或 **我的挑战**（收藏例句，挖空后输入；有合格干扰项数据时才开四选一）。一局最多 30 句；题库够 30 且全部过完才给奖杯。整句输入可配合手机语音转文字软件（推荐豆包语音输入）使用：对着说出英文、自动上屏，练的是真正的说英语，而不是打字写英语。已开通 AI 时另有「挑战者」：从已过关知识点随机出题，自由造句 3 句合格击败一个知识点，连战刷最高纪录。第四扇门是「词语接龙」（词库词 ×2 分，词库外真词也能接，判定前先按例句口径清洗标点，历史最高 ≥30 分得奖杯）。
