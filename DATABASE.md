...
- **造句候选：** `grammar_produce_candidates`（举一反三 AI 合格句：`en` / `zh` 两列 + `level_id`）。本机键 `grammar/produce-candidates/v1`，「我的」可复制或下载；配了密钥时 best-effort INSERT。anon 只能 INSERT，不能列表。正式题库仍靠 migration + Skill `grammar-produce-ingest`。
